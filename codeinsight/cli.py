...
def main():
    parser = argparse.ArgumentParser(
        description="CodeInsight: 多维度 Python 代码质量分析工具"
    )
    ...
    coverage_bar = "█" * int(annotation_coverage // 10) + "░" * (
        10 - int(annotation_coverage // 10)
    )
    print(
        f"  {coverage_bar} {annotation_coverage:.1f}% ({result['total_functions']} 个函数)"
    )
    ...
    if args.fix and not unused:
        ...
        print(
            f"  🗑️  未使用导入 ({len(unused)}): {', '.join(unused) if unused else '无'}"
        )
    ...
    if args.show_functions and result["functions"]:
        ...
        for func in result["functions"]:
            ...
            print()
            print(
                f"     返回注解: {'✓' if func.has_return_annotation else '✗'}", end=""
            )
    ...
    if result["annotation_coverage"] < 50:
        print(
            f"  🏷️   类型注解覆盖率较低 ({annotation_coverage:.1f}%)，建议为关键函数添加类型注解"
        )
        suggestions_count += 1
...
def _analyze_directory(directory: Path, args):
    ...
    if summary:
        ...
        print(
            f"  最佳文件: {Path(summary.get('best_file', '')).name} ({summary.get('best_file_score', 0)})"
        )
        print(
            f"  最差文件: {Path(summary.get('worst_file', '')).name} ({summary.get('worst_file_score', 0)})"
        )
    ...
    for file_path, score in files_with_scores[:10]:
        emoji = (
            "⭐"
            if score >= 80
            else "👍" if score >= 60 else "⚠️" if score >= 40 else "❌"
        )
